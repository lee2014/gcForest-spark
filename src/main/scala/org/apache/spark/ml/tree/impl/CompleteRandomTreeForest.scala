...
/**
  ...
  */
private [spark] object CompleteRandomTreeForest extends Logging {
  /**
  ...
  def run(
    ...
    while (nodeStack.nonEmpty) {
      ...
      CompleteRandomTreeForest.selectNodesToSplit(nodeStack, maxMemoryUsage, metadata, rng)
      ...
      CompleteRandomTreeForest.findRandomSplits(baggedInput, metadata, topNodesForGroup, nodesForGroup,
  ...
  /**
    ...
    */
  private[tree] def findRandomSplits(
  ...
  /**
    ...
    */
  private[tree] def binsToRandomSplit(
    ...
    val featureSplitsAndImpurityStats = validFeatureSplits.map { case (featureIndexIdx, featureIndex) =>
      ...
      if (binAggregates.metadata.isContinuous(featureIndex)) {
        ...
        // Find random split.
        val featureSplits = Range(0, numSplits).map { splitIdx =>
        ...
        val random_index = rand.nextInt(featureSplits.length)
        val (randomFeatureSplitIndex, randomFeatureGainStats) = featureSplits(random_index)
        (splits(featureIndex)(randomFeatureSplitIndex), randomFeatureGainStats)
      } else if (binAggregates.metadata.isUnordered(featureIndex)) {
        // Unordered categorical feature
        val leftChildOffset = binAggregates.getFeatureOffset(featureIndexIdx)
        val featureSplits = Range(0, numSplits).map { splitIndex =>
        ...
        val categoriesForSplit =
          categoriesSortedByCentroid.map(_._1.toDouble).slice(0, randomFeatureSplitIndex + 1)
        val randomFeatureSplit =
          new CategoricalSplit(featureIndex, categoriesForSplit.toArray, numCategories)
        (randomFeatureSplit, randomFeatureGainStats)
      }
    }
  ...
  private[tree] def selectNodesToSplit(
    ...
    while (nodeStack.nonEmpty && (memUsage < maxMemoryUsage || memUsage == 0)) {
      ...
      val nodeMemUsage = CompleteRandomTreeForest.aggregateSizeForNode(metadata, featureSubset) * 8L
